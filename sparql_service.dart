...
class SparqlService {
  static const String endpoint = 'https://fuseki.hillhouse.tech/History/query';
...
SELECT ?obj ?titleLabel ?address ?lat ?lon ?snippetVal ?mainTextVal ?linksVal ?imageVal
       ?creator ?creatorLabel ?constructionYearVal
WHERE {
...
''';
    ...
    for (final item in results) {
      ...
      final constructionYear =
          (row['constructionYearVal']?['value'] ?? '').toString();
      if ((obj['constructionYear'] as String).isEmpty &&
          constructionYear.isNotEmpty) {
  ...
  Future<List<Map<String, String>>> fetchRelatedObjects(String uri) async {
    const query = r'''
PREFIX crm: <http://www.cidoc-crm.org/cidoc-crm/>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
...
''';
  ...
  Future<Map<String, dynamic>?> fetchCreatorDetails(String uri) async {
    const query = r'''
PREFIX crm: <http://www.cidoc-crm.org/cidoc-crm/>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>

SELECT ?creator ?labelVal ?biographicalNoteVal
       ?birthDateVal ?birthYearVal ?deathDateVal ?deathYearVal
       ?occupationVal ?imageVal
       ?relatedObj ?relatedObjLabel ?relatedObj2 ?relatedObjLabel2
WHERE {
...
''';
    ...
    for (final item in results) {
      ...
      final birthYear =
          (row['birthYearVal']?['value'] ?? '').toString().trim();
      final deathDate =
          (row['deathDateVal']?['value'] ?? '').toString().trim();
      final deathYear =
          (row['deathYearVal']?['value'] ?? '').toString().trim();
      ...
      final relatedLabel1 =
          (row['relatedObjLabel']?['value'] ?? '').toString();
      ...
      final relatedLabel2 =
          (row['relatedObjLabel2']?['value'] ?? '').toString();
